...
기억 나시나요? 함수형 컴포넌트는 매 렌더링 마다 고유의/불변의 props,state, 함수를 갖습니다. 

`setInterval`로 `setCount(count + 1)`을 매 초마다 실행하지만 이 때 우리가 기대한건 `count`가 0, 1, 2, 3...으로 증가하는 것이지만, 저 count는 영원히 __0__ 입니다. 그래서 화면엔 1만 보이는 것이죠. `setInterval`안에서 `console.log(count)`를 실행해 보시면 더 확실히 알 수 있습니다.
...
이제 제대로 동작합니다! 다시 차근차근 살펴봅시다.

```
count + 1
```

위 코드 조각에 대해 생각해봅시다. 이 코드는 `count`에 1을 더해서 리액트에게 __돌려주기 위해__ 사용됐습니다. 
하지만 리액트는 사실 `count`를 이미 알고 있습니다. 우리가 진짜로 리액트에게 알려줘야하는 건 __"지금 값이 뭐든 간에 1을 더해줘"__ 입니다.

## 함수형 업데이트와 Google Docs

동기화에 관해 흥미로운 부분은 다음과 같이 축약할 수 있습니다. 

> 시스템간 "메시지"를 상태와 엮이지 않은채로 유지하고 싶을 때가 있다.

Google Docs에서 문서를 편집하는 것은 실제로 서버에 모든 페이지 내용을 보내는 것이 아닙니다.

대신 사용자가 무엇을 __하고자 했는지__ 그 행동을 표현한 것을 보냅니다.

앞서 함수형 업데이트도 비슷한 철학이 적용됩니다. __이펙트 안에서 필요한 최소한의 정보만 컴포넌트로 전달__ 하는 것이 최적화에 도움이 됩니다.

`setCount(c => c + 1)`은 `setCount(count + 1)`보다 적은 정보를 전달합니다. 왜냐하면 현재의 `count`값에 __오염되지 않았기__ 때문입니다. 그저 행위("증가")를 표현할 뿐입니다.

[리액트로 생각하기 참조](https://ko.reactjs.org/docs/thinking-in-react.html#step-3-identify-the-minimal-but-complete-representation-of-ui-state)

하지만 함수형 업데이트는 매우 제한적인 해결책입니다. 간단한 예제에서는 매우 효과적으로 보입니다. 하지만 상태와 props의 관계가 복잡해진다면 다른 해결책이 필요합니다.
